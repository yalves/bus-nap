...
class PinAnnotation : NSObject, MKAnnotation
{
    ...
    func estaNaHoraDeAcordar(destino: CLLocationCoordinate2D, _ localAtual: CLLocation) -> Bool
    {
        let locationDestino: CLLocation = CLLocation.init(latitude: destino.latitude, longitude: destino.longitude)
        
        return (Double(locationDestino.distanceFromLocation(localAtual)) <= 100.0)
    }
}
